...
def show_predictions():
    ...
    db = get_db()
    cur = db.execute("""select artist, lyrics from predictions order by id desc""")
...
def add_prediction():
    ...
    db = get_db()
    db.execute('INSERT INTO predictions (lyrics, artist) values (?, ?)', [lyrics,artistName])
    db.commit()
    flash('Predition was successfully posted!')
